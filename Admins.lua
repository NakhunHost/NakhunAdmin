if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("The panel is already running!", 0)
    return
end
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "Nakhun Admin is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
CMDs[#CMDs + 1] = {NAME = 'Hydroxide (wave) / Hwave', DESC = 'opens hyroxide for wave'}
CMDs[#CMDs + 1] = {NAME = 'slapbattles / sb', DESC = 'loads herkle hub for sb'}
...
addcmd('Hydroxide (wave)',{'hwave'},function(args, speaker)
	notify("Loading",'Hold on a sec')
...
-- HYDROXIDE IS NOT MADE BY ME! I JUST MODIFIED IT SO IT WORKS WITH WAVE!
end)

addcmd('slapbattles',{'sb'},function(args, speaker)
	notify("Loading",'Hold on a sec')
